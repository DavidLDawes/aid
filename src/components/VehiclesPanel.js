...
const VehiclesPanel = ({ vehicles, shipTechLevel, onUpdate }) => {
    ...
    const removeVehicle = (vehicleType) => {
        const newVehicles = vehicles.map(v => v.vehicle_type === vehicleType
            ? { ...v, quantity: Math.max(0, v.quantity - 1) }
            : v).filter(v => v.quantity > 0);
        onUpdate(newVehicles);
    };
    ...
    return (_jsxs("div", { className: "panel-content", children: [_jsxs("p", { children: ["Configure vehicles carried by the starship. Only vehicles compatible with TL-", shipTechLevel, " are available."] }), _jsxs("p", { children: [_jsx("strong", { children: "Total Service Staff Required:" }), " ", totalServiceStaff] }), _jsx("div", { className: "vehicles-grouped-layout", children: groupedVehicles.map((vehicleGroup, groupIndex) => (_jsx("div", { className: "vehicle-group-row", children: vehicleGroup.map(vehicleType => {
                        ...
                        return (_jsxs("div", { className: "component-item", children: [_jsxs("div", { className: "component-info", children: [_jsx("h4", { children: vehicleType.name }), _jsxs("p", { children: [vehicleType.mass, " tons, ", vehicleType.cost, " MCr"] }), quantity > 0 && (_jsxs("p", { children: [_jsx("strong", { children: "Total:" }), " ", (vehicleType.mass * quantity).toFixed(1), " tons, ", (vehicleType.cost * quantity).toFixed(3), " MCr"] }))] }), _jsxs("div", { className: "quantity-control", children: [_jsx("button", { onClick: () => removeVehicle(vehicleType.type), disabled: quantity === 0, children: "-" }), _jsx("span", { children: quantity }), _jsx("button", { onClick: () => addVehicle(vehicleType), children: "+" })] })] }, vehicleType.type));
