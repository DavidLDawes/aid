...
describe('generateShipPrintContent', () => {
  it('should return a valid HTML document', () => {
    const html = generateShipPrintContent(baseShip, baseMass, baseCost, baseStaff, false, false);
  ...
  it('should escape HTML special characters in ship name', () => {
    ...
    const html = generateShipPrintContent(xssShip, baseMass, baseCost, baseStaff, false, false);
  ...
  it('should escape ampersands and quotes in ship name', () => {
    ...
    const html = generateShipPrintContent(ship, baseMass, baseCost, baseStaff, false, false);
  ...
  it('should combine Pilot/Navigator when combinePilotNavigator is true', () => {
    const html = generateShipPrintContent(baseShip, baseMass, baseCost, baseStaff, true, false);
    expect(html).toContain('Pilot/Navigator');
    expect(html).not.toContain('>Pilot<');
  });

  it('should reduce total crew by 1 when combinePilotNavigator is true', () => {
    const htmlCombined = generateShipPrintContent(baseShip, baseMass, baseCost, baseStaff, true, false);
    const htmlSeparate = generateShipPrintContent(baseShip, baseMass, baseCost, baseStaff, false, false);
    expect(htmlCombined).toContain(`<strong>${baseStaff.total - 1}</strong>`);
    expect(htmlSeparate).toContain(`<strong>${baseStaff.total}</strong>`);
  });

  it('should hide stewards when noStewards is true', () => {
    const staffWithStewards = { ...baseStaff, stewards: 2, total: 6 };
    const htmlWithStewards = generateShipPrintContent(baseShip, baseMass, baseCost, staffWithStewards, false, false);
    const htmlNoStewards = generateShipPrintContent(baseShip, baseMass, baseCost, staffWithStewards, false, true);
    expect(htmlWithStewards).toContain('Stewards');
    expect(htmlNoStewards).not.toContain('Stewards');
  });
  ...
  it('should show gunners row only when gunners > 0', () => {
    ...
    expect(generateShipPrintContent(baseShip, baseMass, baseCost, staffNoGunners, false, false)).not.toContain('Gunners');
    expect(generateShipPrintContent(baseShip, baseMass, baseCost, staffWithGunners, false, false)).toContain('Gunners');
  });

  it('should show medical staff rows only when present', () => {
    const staffWithMedical = { ...baseStaff, nurses: 1, surgeons: 1, techs: 1, total: 7 };
    const html = generateShipPrintContent(baseShip, baseMass, baseCost, staffWithMedical, false, false);
