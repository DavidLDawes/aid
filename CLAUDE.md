...
- `MIN_MANEUVER_DRIVE_TONS` (100): a maneuver drive's mass is floored to this regardless of the percentage math — matters for the smallest fractional levels (e.g. unfloored M-.01 is only 10 tons at the smallest megastructure, 1,000,000 tons). Applied in both `calculateEngineMassAndCost()` branches for `maneuver_drive` only; fractional-drive cost is unaffected by the floor (it's an independent percentage of M-1's cost, not derived from mass), but the integer M-1..M-6 branch floors mass *before* computing cost so `cost = mass × rate` stays consistent. Power plants are never floored this way
